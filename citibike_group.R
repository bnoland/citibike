# citibike_group.R
#
# Detects potential groups of riders in Citi Bike trip data.
...
OutputGroup <- function(group, f) {
    ...
    write.table(group, file=f, col.names=FALSE, append=TRUE, sep=",", dec=".", qmethod="double")
}
...
OutputGroupedData1 <- function(citibike, out.file, start.thresh, stop.thresh, show.progress) {
    ...
    for (i in 1:nrow(citibike)) {
        ...
        if (!is.null(last.row.names) && setequal(current.row.names, last.row.names)) {
            next
        }
        ...
        current.group$group.size <- nrow(current.group)
...
ShowProgress <- function(i, citibike, show.output=TRUE) {
    ...
    #   show.output: If set to TRUE, progress information is outputted, otherwise nothing is
    #                displayed.
...
citibike <- with(citibike, citibike[order(start.station.id, end.station.id, starttime, stoptime), ])
...
citibike$group.size <- NA       # Column for holding group size.
